...
def _evaluate_panel_predictions(
    ...
    metrics.update(stability_metrics(net))
    ...
    metrics["instability"] = max(
        0.0,
        min(1.0, float(metrics["monthly_return_volatility"])),
    )
    robustness = float(
        np.clip(
...
def run_model_experiment(
    ...
    metrics, robustness = _evaluate_panel_predictions(panel, oos.predictions, config)
